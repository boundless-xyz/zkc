...
contract veZKCInvariantTest is StdInvariant, Test {
    ...
    function invariant_TotalStakedMatchesSum() public view {
        ...
        assertEq(
            handlerTotal,
            sumOfUsers,
            "Total staked - unstaked should equal sum of all user stakes"
        );
    }
    ...
    function invariant_LockedAmountConsistency() public view {
        ...
        assertEq(
            totalInContract,
            totalTracked,
            "ZKC balance in veToken should equal tracked total staked"
        );
    }
    ...
    function invariant_NoNegativeVotingPower() public view {
        ...
        for (uint256 i = 0; i < actorCount; i++) {
            ...
            assertTrue(
                votingPower <= type(uint128).max,
                "Voting power overflow detected"
            );
        }
    }
        ...
        for (uint256 i = 0; i < actorCount; i++) {
            ...
            if (handler.ghost_hasActivePosition(actor)) {
                ...
                assertEq(
                    rewardPower,
                    0,
                    "No position should mean zero reward power"
                );
            }
        }
    }
    ...
    function invariant_PastTotalSupplyReasonable() public view {
        ...
        for (uint256 i = 0; i < snapshotCount; i += step) {
            ...
            assertTrue(
                pastSupply <= currentActiveStaked * 100 || pastSupply == 0,
                "Past total supply is unreasonably high"
            );
        }
    }
    ...
    function invariant_SingleActivePositionPerUser() public view {
        ...
        for (uint256 i = 0; i < actorCount; i++) {
            ...
            if (activePosition != 0) {
                ...
                assertTrue(
                    handler.ghost_hasActivePosition(actor),
                    "Active position mismatch with ghost state"
                );
                
                assertEq(
                    activePosition,
                    handler.ghost_userTokenId(actor),
                    "Token ID mismatch with ghost state"
                );
            } else {
                // If no active position, ghost state should reflect that
                assertFalse(
                    handler.ghost_hasActivePosition(actor),
                    "Ghost state shows position when none exists"
                );
            }
        }
    }
