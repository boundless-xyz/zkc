...
contract CirculatingZKCTest is Test {
    ...
    function testInitialization() public view {
        ...
        assertEq(circulatingZKC.locked(), zkc.INITIAL_SUPPLY() - INITIAL_UNLOCKED);
    ...
    function testCirculatingSupplyAfterInitialMint() public {
        ...
        uint256 expectedCirculating = claimedTotal - circulatingZKC.locked();
    ...
    function testUpdateUnlockedValue() public {
        ...
        assertEq(circulatingZKC.locked(), expectedLocked);
        // Verify synchronization: locked + unlocked = INITIAL_SUPPLY
        assertEq(circulatingZKC.locked() + circulatingZKC.unlocked(), zkc.INITIAL_SUPPLY());

        // Check circulating supply updated correctly
        uint256 circulatingSupply = circulatingZKC.circulatingSupply();
        uint256 claimedTotal = zkc.claimedTotalSupply();
        assertEq(circulatingSupply, claimedTotal - circulatingZKC.locked());
    }
    ...
    function testUpdateUnlockedValueAccessControl() public {
        ...
        circulatingZKC.updateUnlockedValue(newUnlocked);
        assertEq(circulatingZKC.unlocked(), newUnlocked);
        assertEq(circulatingZKC.locked(), zkc.INITIAL_SUPPLY() - newUnlocked);
    }
    ...
    function testUpdateLockedValue() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit CirculatingZKC.LockedValueUpdated(circulatingZKC.locked(), newLocked);
        vm.prank(owner);
        circulatingZKC.updateLockedValue(newLocked);

        assertEq(circulatingZKC.locked(), newLocked);
        assertEq(circulatingZKC.unlocked(), expectedUnlocked);
        // Verify synchronization: locked + unlocked = INITIAL_SUPPLY
        assertEq(circulatingZKC.locked() + circulatingZKC.unlocked(), zkc.INITIAL_SUPPLY());
    ...
    function testUpdateLockedValueAccessControl() public {
        ...
        circulatingZKC.updateLockedValue(newLocked);
        assertEq(circulatingZKC.locked(), newLocked);
        assertEq(circulatingZKC.unlocked(), zkc.INITIAL_SUPPLY() - newLocked);
    }
