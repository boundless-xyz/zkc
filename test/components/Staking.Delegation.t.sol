...
contract StakingDelegationTest is veZKCTest {
    ...
    function testAddToStakeIncreasesRewardDelegation() public {
        ...
        assertEq(bobRewardsAfter, (AMOUNT * 2) / Constants.REWARD_POWER_SCALAR, "Bob should have increased reward power");
    }
    ...
    function testAddToStakeWithSplitDelegation() public {
        ...
        assertEq(veToken.getStakingRewards(CHARLIE), (AMOUNT * 2) / Constants.REWARD_POWER_SCALAR, "Charlie should have increased reward power");
    ...
    function testComplexDelegationScenario() public {
        ...
        assertEq(veToken.getStakingRewards(DAVE), AMOUNT / Constants.REWARD_POWER_SCALAR, "Dave should have only Alice rewards");
        ...
        assertEq(veToken.getStakingRewards(bob), AMOUNT / Constants.REWARD_POWER_SCALAR, "Bob should have Alice's rewards");
    }
    ...
    function testDelegationPersistsThroughMultipleStakes() public {
        ...
        assertEq(veToken.getStakingRewards(CHARLIE), (AMOUNT * 4) / Constants.REWARD_POWER_SCALAR, "Charlie should have 4x reward power");
    }
    ...
    function testZeroAddressDelegationBehavior() public {
        ...
        assertEq(veToken.getStakingRewards(alice), AMOUNT / Constants.REWARD_POWER_SCALAR, "Alice should have reward power");
    ...
    function testComplexDelegationChains() public {
        ...
        assertEq(veToken.getStakingRewards(users[1]), (AMOUNT * 4) / Constants.REWARD_POWER_SCALAR, "users[1] should have users[3]'s rewards");
        // users[2] delegates to themselves, keeps their own (3 * AMOUNT)
        assertEq(veToken.getStakingRewards(users[2]), (AMOUNT * 3) / Constants.REWARD_POWER_SCALAR, "users[2] should have their own rewards");
        // users[3] receives users[1]'s delegation (2 * AMOUNT)
        assertEq(veToken.getStakingRewards(users[3]), (AMOUNT * 2) / Constants.REWARD_POWER_SCALAR, "users[3] should have users[1]'s rewards");
        // users[4] receives users[0]'s delegation (1 * AMOUNT) + keeps their own (5 * AMOUNT) = 6 * AMOUNT
        assertEq(veToken.getStakingRewards(users[4]), (AMOUNT * 6) / Constants.REWARD_POWER_SCALAR, "users[4] should have accumulated rewards");
    }
}