...
contract ZKCEpochsTest is ZKCTest {
    ...
    function testGetEpochEndTime() public view {
        assertEq(zkc.getEpochEndTime(0), deploymentTime + zkc.EPOCH_DURATION() - 1);
        assertEq(zkc.getEpochEndTime(1), deploymentTime + 2 * zkc.EPOCH_DURATION() - 1);
        
        uint256 epoch = 100;
        uint256 expectedEndTime = deploymentTime + (epoch + 1) * zkc.EPOCH_DURATION() - 1;
    ...
    function testEpochConstants() public view {
        assertEq(zkc.EPOCH_DURATION(), 2 days);
        assertEq(zkc.EPOCHS_PER_YEAR(), 182);
        assertEq(zkc.POVW_ALLOCATION_BPS(), 7500); // 75%
        assertEq(zkc.STAKING_ALLOCATION_BPS(), 2500); // 25%
        assertEq(zkc.BASIS_POINTS(), 10000);
    }
    ...
    function testSupplyGrowthOverTime() public view {
        ...
        // Supply should grow over time due to emissions
