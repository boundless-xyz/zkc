    ...
    IERC6372
{
    ...
    function _mintLock(address to, uint256 amount, uint256 lockWeeks) internal returns (uint256) {
        require(_isValidLockWeeks(lockWeeks), "Invalid lock weeks");

        uint256 tokenId = ++_currentTokenId;
        _mint(to, tokenId);

        locks[tokenId] = LockInfo({amount: amount, lockEnd: block.timestamp + (lockWeeks * WEEK), lockWeeks: lockWeeks});
    ...
    function _addStake(uint256 tokenId, uint256 newAmount) internal {
    ...
    function _extendLock(uint256 tokenId, uint256 newLockEndTime) internal {
        require(_ownerOf(tokenId) != address(0), "Token does not exist");
    ...
    function _extendLockByWeeks(uint256 tokenId, uint256 additionalWeeks) internal {
        require(_ownerOf(tokenId) != address(0), "Token does not exist");
    ...
    function _delegate(address account, address delegatee) internal {
        ...
        if (oldDelegate != delegatee && oldDelegate != address(0)) {
            ...
            _delegateCheckpoints[oldDelegate].push(clock(), (oldDelegateVotes - accountVotes).toUint208());
        }

        // Update new delegate (add account's voting power)
        if (delegatee != address(0)) {
            uint256 newDelegateVotes = _delegateCheckpoints[delegatee].latest();
            _delegateCheckpoints[delegatee].push(clock(), (newDelegateVotes + accountVotes).toUint208());
        }
    ...
    function stake(uint256 amount, uint256 lockWeeks) external nonReentrant returns (uint256 tokenId) {
        ...
        require(userActivePosition[user] == 0, "User already has an active position");
        ...
        tokenId = _mintLock(user, amount, lockWeeks);

        // Track user's active position
        userActivePosition[user] = tokenId;
    ...
    function extendLockToTime(uint256 tokenId, uint256 newLockEndTime) external nonReentrant {
        ...
        _extendLock(tokenId, newLockEndTime);
    ...
    function extendLockByWeeks(uint256 tokenId, uint256 additionalWeeks) external nonReentrant {
        ...
        _extendLockByWeeks(tokenId, additionalWeeks);
