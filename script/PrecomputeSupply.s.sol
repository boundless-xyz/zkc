...
contract PrecomputeSupply is Script {
    ...
    function run() external {
        ...
        for (uint256 i = 0; i < epochFactors.length; i++) {
            console.log("uint256 public constant Y%s_R_PER_EPOCH = %s; // Year %s", 
                i, epochFactors[i], i);
        }
        
        console.log("");
        console.log("uint256 public constant FINAL_R_PER_EPOCH = %s; // 3.0%% annual", 
            epochFactors[8]);
        ...
        for (uint256 i = 0; i < supplyBoundaries.length; i++) {
            if (i == 0) {
                console.log("uint256 public constant SUPPLY_YEAR_%s = %s; // Initial supply", 
                    i, supplyBoundaries[i]);
            } else {
                uint256 epoch = i * EPOCHS_PER_YEAR;
                console.log("uint256 public constant SUPPLY_YEAR_%s = %s; // Supply at epoch %s",
                    i, supplyBoundaries[i], epoch);
            }
        }
    }
}